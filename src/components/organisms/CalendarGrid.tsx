...
import { localDateISO, formatLongDate } from '../../utils';
...
export function CalendarGrid({ entries, onEdit, onDelete, onAddForDate }: CalendarGridProps) {
  ...
  return (
    <>
      <div className={styles.shell}>
        ...
        <div className={styles.grid}>
          {cells.map((day) => {
            ...
            const cellEntries = entries.filter((e) => e.date === iso);
              ...
              >
                ...
                {cellEntries.length > 0 && (
                  <span className={styles.count}>
                    {cellEntries.length}{' '}
                    {cellEntries.length === 1 ? 'actividad' : 'actividades'}
                  </span>
                )}
              </button>
            );
      ...
      <div className={styles.sectionTitle}>
        <span className={styles.dot2} style={{ background: '#7C9FFF' }} />
      ...
      <div className={styles.legend}>
        {CALENDAR_TYPE_ORDER.map((type) => (
          <div key={type} className={styles.legendItem}>
            ...
            />
            <span>{TYPE_INFO[type].icon} {TYPE_INFO[type].label}</span>
          </div>
        ))}
      </div>
      ...
      <EntryList entries={dayEntries} onEdit={onEdit} onDelete={onDelete} />
    </>
  );
}
